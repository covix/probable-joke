...
grep '] Solving ' $1 > aux_seconds_train_raw.txt
grep 'Testing net (#0)' $1 >> aux_seconds_train_raw.txt
$DIR/extract_seconds.py aux_seconds_train_raw.txt aux_seconds_train.txt

grep '] Solving ' $1 > aux_seconds_test_raw.txt
grep 'Testing net (#1)' $1 >> aux_seconds_test_raw.txt
$DIR/extract_seconds.py aux_seconds_test_raw.txt aux_seconds_test.txt

# Generating train acc
echo '#Iters Seconds TestAccuracy1 TestAccuracy5 TestLoss'> $LOG.test_on_train
paste aux0.txt aux_seconds_train.txt aux_train_acc1.txt aux_train_acc5.txt aux_train_loss.txt | column -t >> $LOG.test_on_train
rm aux_train.txt aux_train_acc1.txt aux_train_acc5.txt aux_train_loss.txt aux_seconds_train_raw.txt aux_seconds_train.txt

# Generating test acc
echo '#Iters Seconds TestAccuracy1 TestAccuracy5 TestLoss'> $LOG.test_on_test
paste aux0.txt aux_seconds_test.txt aux_test_acc1.txt aux_test_acc5.txt aux_test_loss.txt | column -t >> $LOG.test_on_test
rm aux.txt aux_test.txt aux0.txt aux_test_acc1.txt aux_test_acc5.txt aux_test_loss.txt aux_seconds_test_raw.txt aux_seconds_test.txt aux.txt.bak
