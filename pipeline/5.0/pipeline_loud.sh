...
if [[ ! -d $LOUD_ALIGNMENT_INDEXES_TEST ]]; then
    ...
    _WAIT_ID="$ANTERIOR_LOUD_PCA_FEATURES_CLASS_TRAIN $ANTERIOR_LOUD_PCA_FEATURES_CLASS_TEST"

    CMD="$ALIGNMENT_SCRIPTS_FOLDER/run_align_deep_pca_gctw_test_correct_class.sh $LOUD_PCA_FEATURES_CLASS_TRAIN $LOUD_PCA_FEATURES_CLASS_TEST $LOUD_ALIGNMENT_INDEXES_TEST ${_WAIT_ID}"
    OAR_SUB_OUTPUT=`"$CMD"`
